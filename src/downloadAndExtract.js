...
async function ensureInStore(name, version, tarballMeta) {
  ...
  if (tarballMeta.integrity) {
    ...
    if (fileSha512 !== expected) {
      console.error(
        `[SECURITY] Integrity (sha512) mismatch for ${name}@${version}: expected ${expected}, got ${fileSha512}`,
      );
      hashOk = false;
    } else {
      console.log(
        `[SECURITY] Integrity (sha512) verified for ${name}@${version}`,
      );
    }
  }
  if (tarballMeta.shasum) {
    ...
    if (fileSha1 !== tarballMeta.shasum) {
      console.error(
        `[SECURITY] Hash (sha1) mismatch for ${name}@${version}: expected ${tarballMeta.shasum}, got ${fileSha1}`,
      );
      hashOk = false;
    } else {
  ...
  if (tarballMeta.signature) {
    ...
    console.log(
      `[SECURITY] Signature present for ${name}@${version} (not yet verified)`,
    );
