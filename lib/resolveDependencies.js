    ...
    async function worker([name, versionRange]) {
        ...
        if (typeof versionRange === "string" && versionRange.startsWith("npm:")) {
            ...
            if (match) {
                ...
                if (worker._aliasChain.length >= MAX_ALIAS_DEPTH) {
                    throw new Error(`Alias chain too deep (>${MAX_ALIAS_DEPTH}): ${[...worker._aliasChain, name].join(" -> ")}`);
                }
        ...
        if (pkg.peerDependencies) {
            for (const [peerName, peerRange] of Object.entries(pkg.peerDependencies)) {
                ...
                if (!found) {
                    peerWarnings.push(`Peer dependency missing: ${name}@${resolvedVersion} requires ${peerName}@${peerRange}`);
                }
                else if (!semver.satisfies(found.version, peerRange)) {
                    peerWarnings.push(`Peer dependency version mismatch: ${name}@${resolvedVersion} requires ${peerName}@${peerRange}, found ${found.version}`);
                }
            }
        }
        ...
        if (pkg.optionalDependencies) {
            for (const [optName, optRange] of Object.entries(pkg.optionalDependencies)) {
                try {
                    ...
                    optionalWarnings.push(`Optional dependency failed: ${name}@${resolvedVersion} optional ${optName}@${optRange} (${err.message})`);
                }
            }
        }
