...
const { runLifecycleScript } = require('./installTree');
...
async function ensureInStore(name, version, tarballMeta) {
    ...
    try {
        ...
        if (isVerbose) {
            console.log(chalk.green(`[STORE] Using existing global store for ${name}@${version}`));
        }
        return storePath; // Already extracted, skip!
    } catch {
    ...
    if (tarballMeta.integrity) {
        ...
        if (fileSha512 !== expected) {
            if (isVerbose)
                console.error(chalk.red(`[SECURITY] Integrity (sha512) mismatch for ${name}@${version}: expected ${expected}, got ${fileSha512}`));
            hashOk = false;
        }
        else {
            if (isVerbose)
                console.log(chalk.green(`[SECURITY] Integrity (sha512) verified for ${name}@${version}`));
        }
    }
    if (tarballMeta.shasum) {
        ...
        if (fileSha1 !== tarballMeta.shasum) {
            if (isVerbose)
                console.error(chalk.red(`[SECURITY] Hash (sha1) mismatch for ${name}@${version}: expected ${tarballMeta.shasum}, got ${fileSha1}`));
            hashOk = false;
        }
        else {
            if (isVerbose)
                console.log(chalk.green(`[SECURITY] Hash (sha1) verified for ${name}@${version}`));
        }
    }
    if (!hashOk) {
    ...
    if (tarballMeta.signature) {
        if (isVerbose)
            console.log(chalk.yellow(`[SECURITY] Signature present for ${name}@${version} (not yet verified)`));
    ...
    if (isVerbose) {
        console.log(chalk.cyan(`[STORE] Extracted ${name}@${version} to global store`));
    }
    // Run lifecycle scripts only after extraction
    await runLifecycleScript(storePath, "preinstall", name);
    await runLifecycleScript(storePath, "install", name);
    await runLifecycleScript(storePath, "postinstall", name);
    return storePath;
}
