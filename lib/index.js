...
const { parseArgs, safeReadPackageJson, validateCommand } = require("./cli");
...
async function main(args) {
    ...
    // Welcome banner
    console.log(boxen(`${chalk.bold.cyan('🚀 blaze-install')} ${chalk.gray(`v${version}`)}`, { padding: 1, margin: 1, borderStyle: 'round', borderColor: 'cyan', align: 'center' }));
    ...
    try {
        ...
        const { command, packages, flags, workspaceTarget } = parseArgs(args);
        // Help command
        ...
        if (command === "prefetch") {
            ...
            const rootPkg = await safeReadPackageJson();
            let depsToPrefetch = {};
        ...
        if (command === "uninstall") {
            validateCommand("uninstall", packages, "<package>");
            // Plugin hook
            for (const plugin of plugins) {
            ...
            const pkgDir = path.join(process.cwd(), "node_modules", packages[0]);
            await runLifecycleScript(pkgDir, "preuninstall", packages[0]);
            await runLifecycleScript(pkgDir, "uninstall", packages[0]);
            await uninstallPackage(packages[0]);
            await runLifecycleScript(pkgDir, "postuninstall", packages[0]);
            await pruneLockfile();
        ...
        if (command === "update") {
            ...
            validateCommand("update", packages, "<package>");
            const pkg = await safeReadPackageJson();
            const isDev = pkg.devDependencies && pkg.devDependencies[packages[0]];
            await updatePackage(packages[0], isDev);
            // Plugin hook
            for (const plugin of plugins) {
        ...
        if (command === "install" || command === undefined) {
            const { offline, auditFix, noLockfile, production, saveDev, ci } = flags;
            ...
            if (addingPackage && packages[0]) {
                ({ name: pkgName, range } = parsePackageArg(packages[0]));
            ...
            let rootPkgLatest = await safeReadPackageJson();
            let depsToInstall = {};
            let workspacePaths = [];
            const ciMode = ci;
            if (ciMode) {
            ...
            if (addingPackage) {
                ...
                if (Object.keys(depsToInstall).length > 0) {
                    ...
                    if (args.includes("--debug")) {
                        ...
                        console.log(chalk.gray(`[DEBUG] Dependency tree resolved. Top-level packages: ${Object.keys(tree).join(", ")}`));
                    }
                ...
                console.log(chalk.blue("✨ No blaze-lock.json found or lockfile skipped. Resolving dependencies..."));
                if (Object.keys(depsToInstall).length > 0) {
                    const spinner = new Spinner("Resolving dependencies");
                    spinner.start();
                    const tree = await resolveDependencies(depsToInstall, {}, null, [], [], { offline });
            ...
            if (auditFix && noLockfile) {
                console.error(chalk.red("Error: --audit-fix cannot be used with --no-lockfile. The audit requires a lockfile."));
                process.exit(1);
            }
            if (offline) {
            ...
            console.log(boxen(chalk.bold.white(`Installation Summary`) +
        ...
        console.error(chalk.red("❌ Error:"), err && err.stack ? chalk.red(err.stack) : chalk.red(err));
        process.exit(1);
    }
}
