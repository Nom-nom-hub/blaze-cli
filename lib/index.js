...
async function main(args) {
  try {
    ...
    if (command === 'install' || command === undefined) {
      ...
      // Block --audit-fix with --no-lockfile before any install logic
      ...
      let newPackageAdded = false;
      if (rest.length > 1) {
        const { name: pkgName, range } = parsePackageArg(rest[1]);
      ...
      if (ciMode) {
        // Remove node_modules for a clean slate
      ...
      // FIX: Always resolve dependencies and update lockfile if a new package was added
      if (!newPackageAdded && lock && !noLockfile) {
        ...
        console.log('No blaze-lock.json found, local dependencies present, new package added, or lockfile skipped. Resolving dependencies...');
      ...
      // Run auditAndFix if --audit-fix is present
      if (auditFix && noLockfile) {
        console.error('Error: --audit-fix cannot be used with --no-lockfile. The audit requires a lockfile.');
        process.exit(1);
      }
      if (auditFix) {
        console.log('Running security audit and fix...');
        await auditAndFix();
      }
      return;
    }
