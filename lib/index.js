...
async function auditPackages() {
  ...
  try {
    ...
    if (data.metadata && data.metadata.vulnerabilities && data.metadata.vulnerabilities.total === 0) {
      console.log((await getChalk()).green('No known vulnerabilities found!'));
      return;
    }
    if (data.advisories) {
      let found = 0;
      for (const key in data.advisories) {
        ...
        found++;
        console.log((await getChalk()).red.bold(`VULNERABILITY: ${advisory.module_name}@${advisory.findings[0].version}`));
        console.log((await getChalk()).red(`  Severity: ${advisory.severity}`));
        console.log((await getChalk()).yellow(`  Title: ${advisory.title}`));
        console.log((await getChalk()).gray(`  URL: ${advisory.url}`));
        console.log((await getChalk()).gray(`  Vulnerable: ${advisory.vulnerable_versions}`));
        console.log();
      }
      console.log((await getChalk()).red.bold(`Found ${found} vulnerable packages!`));
    } else {
      console.log((await getChalk()).green('No known vulnerabilities found!'));
    }
  } catch (err) {
  ...
  try {
    ...
    if (command === 'list') {
      ...
      if (Object.keys(deps).length === 0) {
        ...
        for (const [name, version] of Object.entries(deps)) {
          console.log((await getChalk()).green(`  ${name}@${version}`));
        }
      }
      ...
      if (Object.keys(devDeps).length === 0) {
        ...
        for (const [name, version] of Object.entries(devDeps)) {
          console.log((await getChalk()).cyan(`  ${name}@${version}`));
        }
      }
      console.log();
      return;
    ...
    if (command === 'outdated') {
      ...
      for (const [name, current] of Object.entries(all)) {
        try {
          ...
          console.log(pad(name, 25) + pad(current, 15) + pad('-', 15) + (await getChalk()).yellow('Error fetching latest'));
        }
      }
      console.log();
      return;
    ...
    if (command === 'info') {
      ...
      try {
        ...
        if (info.description) console.log((await getChalk()).green('Description:'), info.description);
        if (info.homepage) console.log((await getChalk()).green('Homepage:'), info.homepage);
        if (info.repository && info.repository.url) console.log((await getChalk()).green('Repository:'), info.repository.url.replace(/^git\+/, ''));
        if (info.license) console.log((await getChalk()).green('License:'), info.license);
        if (info.maintainers) {
    ...
    if (command === 'install' || command === undefined) {
      ...
      console.log('[DEBUG] Entered install command logic (latest code)');
      ...
      if (addingPackage && rest[0]) {
        ...
        console.log(`[DEBUG] Dependency added to: ${targetPkgPath}`);
      }
      ...
      if (rootPkgLatest.workspaces && Array.isArray(rootPkgLatest.workspaces)) {
        ...
        if (production) {
          ...
          depsToInstall = { ...(rootPkgLatest.dependencies || {}), ...(rootPkgLatest.devDependencies || {}), ...allDeps, ...allDevDeps };
        }
      } else {
        if (production) {
              ...
              const { autoInstall } = await inquirer.prompt([
                {
                  type: 'confirm',
                  name: 'autoInstall',
                  message: `Auto-install missing peer dependencies (${missingPeers.join(', ')})?`,
                  default: false
                }
              ]);
              if (autoInstall) {
