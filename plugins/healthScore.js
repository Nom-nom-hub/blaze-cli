...
function getLicenseIssues(pkg) {
  ...
  for (const [name, version] of Object.entries(pkg.dependencies || {})) {
    ...
    if (name === 'left-pad') nonAllowed.push({ name, license: 'WTFPL' });
  }
  return nonAllowed;
}
...
module.exports = {
  onCommand: async ({ command }) => {
    if (command === 'install' || command === 'update') {
      ...
      if (outdated.length) score -= outdated.length * 5;
      const unused = getUnusedDeps(pkg, path.join(process.cwd(), 'node_modules'));
      if (unused.length) score -= unused.length * 2;
      const licenseIssues = getLicenseIssues(pkg);
      if (licenseIssues.length) score -= licenseIssues.length * 10;
      ...
      if (score === 100) {
        ...
        if (outdated.length) console.log('    - Update outdated dependencies.');
        if (unused.length) console.log('    - Remove unused dependencies.');
        if (licenseIssues.length) console.log('    - Review flagged licenses.');
      }
    }
  },
};
