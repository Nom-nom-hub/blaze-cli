...
function setupPreCommitHook() {
  ...
  if (fs.existsSync('.git') && (!fs.existsSync(hookPath) || !fs.readFileSync(hookPath, 'utf8').includes('preCommitDepChecker'))) {
...
function checkDeps() {
  try {
    const outdated = execSync('node bin/blaze-install.js outdated --json', { encoding: 'utf8' });
