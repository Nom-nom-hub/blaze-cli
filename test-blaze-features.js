...
try {
  ...
  pkg.scripts = { preinstall: 'echo preinstall', install: 'echo install', postinstall: 'echo postinstall' };
  fs.writeFileSync('package.json', JSON.stringify(pkg, null, 2));
  const out = run(`${BLAZE} install`);
  assert(/preinstall|install|postinstall/i.test(out));
...
// 11. Dependency graph
try {
  const out = run(`${BLAZE} graph`);
  assert(/graph|mermaid/i.test(out));
...
// 15. CLI help/docs
try {
  const out = run(`${BLAZE} --help`);
  assert(/usage|command/i.test(out));
